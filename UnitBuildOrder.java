...
public class UnitBuildOrder {
	 ...
	 * int factoryId = factory.id(); VectorField field = new VectorField();
	 * field.setTarget(loc); for (int i = 0; i < unitsMaking.size(); i++) {
	 * deployUnits(gc, factory); int deployedUnit = unitsMaking.get((long)i);
	 * Direction moveTowards = field.getDirection(loc); if(gc.canMove(deployedUnit,
	 * moveTowards) && gc.isMoveReady(deployedUnit)) { gc.moveRobot(deployedUnit,
	 * moveTowards); } } }
	 */
}